...
| `join.html` | Recruitment: how SBC Week works, eligibility, resume resources, FAQ, mailing list |
...
**Update recruitment** (`join.html`): the page has two states. **Closed** (current): the hero says recruitment has closed, `#apply` describes how SBC Week generally works with no dates, the `apply-box` and hero button point at the mailing list, and the home page hero has no notice. **Open**: add the SBC Week schedule back as an `<ol class="tl">` in a `#schedule` section (one `<li class="tl__item">` per event: day, date, time, title, blurb, room; give the deadline `tl__item--deadline`), put the dates in the hero, the schedule heading, step four, and the `apply-box`, point the hero button, step four, and the `apply-box` button at the application form, and add the "applications are open" notice (`.hero__notice`) to the home page hero. The 2026 schedule, hero copy, home notice, and apply box are saved verbatim in `reference/sbc-week-2026-schedule.html` (not linked from the site) to copy from. Resume resources (the Google Docs template, the `.docx` copy in `assets/docs/`, and the sample-resume link) live in the `#resume` block just above the `apply-box`. The application FAQ is the `#faq` section below it: an `.accordion.accordion--faq` with one `<details>` per question (summary text plus an `.accordion__body` answer); add or edit questions there.
